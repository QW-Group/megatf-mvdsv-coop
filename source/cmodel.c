...
qbool RecursiveHullTrace (int num, float p1f, float p2f, vec3_t p1, vec3_t p2)
{
...
// move up to the node
	...
	if (CM_HullPointContents (hull, node->children[side^1], mid)
	!= CONTENTS_SOLID)
// go past the node
...
//==================
	...
	while (CM_HullPointContents (hull, hull->firstclipnode, mid)
	== CONTENTS_SOLID)
	{ // shouldn't really happen, but does occasionally
...
** CM_LoadMap
*/
cvar_t	sv_map_halflife	= { "sv_map_halflife", "0"};
extern qbool map_hlcrouch;
cmodel_t *CM_LoadMap (char *name, qbool clientload, unsigned *checksum, unsigned *checksum2)
{
	...
	map_hlcrouch = (i == HL_BSPVERSION);
	if (map_hlcrouch)
		Cvar_Set(&sv_map_halflife, "1");
	else
		Cvar_Set(&sv_map_halflife, "0");
