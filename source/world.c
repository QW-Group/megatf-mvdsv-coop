...
hull_t *SV_HullForEntity (edict_t *ent, vec3_t mins, vec3_t maxs, vec3_t offset)
{
	...
	{	// explicit hulls in the BSP model
		...
		starg = PR2_GetString(ent->v.target);
		stargname = PR2_GetString(ent->v.targetname);
		if (!model)
			SV_Error ("SOLID_BSP with a non-bsp model: classname: %s, netname: %s, target: %s, targetname: %s, model num: %i", sta, PR_GetString(ent->v.netname), starg, stargname, SV_ModelIndex(PR_GetString(ent->v.model)));
		...
		{
			if (size[2] < 54 && model->hulls[3].clip_maxs[0])
...
// touch linked edicts
	for (i = 0; i < numtouch; i++)
	{
		...
		if (touch->v.solid == SOLID_TRIGGER && !((int)touch->v.flags & FL_FINDABLE_NONSOLID))
			continue;		
		// CP
		if (touch->v.solid == SOLID_TRIGGER) // xav: orig
