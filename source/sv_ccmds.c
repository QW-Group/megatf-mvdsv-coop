...
// to allow to store to seperate directory (for IIS perhaps?)
cvar_t	if_cfgdir = {"if_cfgdir", ""};
...
FIXME: make system time independent (modifying system time will generate improper uptime)
============
*/
void SV_Uptime_f (void)
{
	time_t			day, hr, min/*, sec*/;
	...
	{ 
		// days
		day = (int)(now / 86400);
		sprintf (d, "%d day", (int)day);
		if ((int)(now / 86400) >= 2)
			strcat (d, "s "); else strcat (d, " "); // pluralize 
		now -= (((int)(now / 86400)) * 86400);
	}
	else 
		day = 0;

	hr = (time_t) ((int)now / 3600); // hours
	now -= (hr * 3600);
	min = (time_t) ((int)now / 60); // minutes
/*	now -= (min * 60);
	sec = (time_t) ((int)now);	// seconds 
*/
	Con_Printf ("uptime: %s%02d hr%s%02d min%s\n"/*%02d secs\n"*/, d,
		(int)hr, (int)hr > 1 ? "s " : " ",
		(int)min, (int)min > 1 ? "s" : " ");
...
Lists maps in the maps\ directory in current gamedir.
FIXME: list from pak files.
==================
*/
// XavioR's words of wisdom: zomg, no client maplist?! I must surely remedy! (to cmd.c, robin!)
...
void SV_SkinList_f (void)
{
	...
	Con_Printf ("\n"); //ugh @ timestamps
	if (strcmp(key, ""))
		Con_Printf ("Wildcard total:  %d skins, %.1fMB\n", pi, (float)ps/(1024*1024));
...
void SV_KickClient (client_t *cl, qbool ban)
{
	int			cs, l,i;
	char		*s, *r = NULL;
	char		reason[80];
	...
	if (cs > 2)
	{
		...
		if (r == NULL)
			...
			l = strlen(Cmd_Argv(1)) + 1; // uid & space

			// move pointer to location in memory where reason begins
			for (i=1 ; *r, i<=l ; r++, i++) 
				;

			Q_snprintf (reason, sizeof(reason), ": %s", r);
	}

	cs = cl->state;

	cl->state = cs_free;
	SV_BroadcastPrintf (PRINT_HIGH, "%s was kicked%s%s\n", cl->name,
		ban ? " & banned" : "", reason);
	cl->state = cs;

	SV_ClientPrintf (cl, PRINT_HIGH, "You were kicked%s%s\n",
		ban ? " & banned" : "", reason);
	...
	if (ban)
	{
		...
		if (reason[0])
			Q_snprintf (newreason, sizeof(newreason), va("%s%s", cl->name, reason));
		else
			Q_snprintf (newreason, sizeof(newreason), cl->name);

		IF_AddFilter (IF_BAN, 0, s, newreason);
	}

	SV_DropClient (cl);
}

void SV_ForceRemove (qbool ban)
{
	client_t	*cl;
	int			i, uid;
	...
	switch (sv_redirected)
	{
		case RD_MOD:
			if (is_ktpro)
			{
				...
				for (i = 0, cl = svs.clients; i < MAX_CLIENTS; i++, cl++)
				{
					...
					// XavioR: if downloading, print percentage
					if (cl->download)
					{
						Com_Printf ("%3d%% \n", cl->downloadcount * 100 / cl->downloadsize);
					}
					else
					{
						switch (cl->state)
		...
		case RD_NONE:
			...
			for (i = 0, cl = svs.clients; i < MAX_CLIENTS; i++, cl++)
			{
				...
				if (cl->download)
				{
					Com_Printf ("%3d%% ", cl->downloadcount * 100 / cl->downloadsize);
				}
				else
				{
					switch (cl->state)
...
// XavioR: Print bots too
	for (i=0,cl=&sbi.bots[i] ; i<MAX_CLIENTS ; i++,cl++)
	{
		if ( cl->state == 666 )
		{
			if ( cl->botent && cl->botent->v.team != 0) {
/*				Con_Printf ("%-18s %4s %5i %6s %s\n", cl->bname, "BOT",
							2, "hoo",
							"HEH");
*/
				Con_Printf ("%-16s %4i %5i %6i %-22s\n", cl->bname, 0,
						(int)cl->botent->v.frags, cl->buserid, "BOT");
			}
		}
	}
	Con_Printf ("\n");
...
void SV_Check_maps_f(void)
{
	...
	for (; list->name[0] && i <= LOCALINFO_MAPS_LIST_END; list++)
	{
		...
		key = va("%d", i);
		Info_SetValueForKey (localinfo, key, list->name, MAX_SERVERINFO_STRING);
	...
	{
		key = va("%d", i);
		Info_SetValueForKey(localinfo, key, "", MAX_SERVERINFO_STRING);
...
void SV_ConSay_f(void)
{
	...
	// send to chat log
	// log whether it was a local user or invoked remotely
	// if a user used chat via rcon account, log what the account name is that said it
	if (pRconCurPos != NULL)
		Com_Log (CHATLOG, LO_TIMESTAMP|LO_FORMAT, "[remote user \"%s\"] ", pRconCurPos->name);
	else
		Com_Log (CHATLOG, LO_TIMESTAMP, "[local user] ");
	
	Com_Log (CHATLOG, LO_NONE, text);
...
Lists all ban filters and any and/reason if applicable
=============
*/
void IF_FilterList (int type);
static void SV_BanFilterList_f (void)
{	
	IF_FilterList (IF_BAN);
}
...
=============
*/
static void SV_LoadIPList_f (void)
{
	char		path[MAX_QPATH];

	if (if_cfgdir.string[0])
		Q_snprintf (path, sizeof(path), "%s/listip.cfg", if_cfgdir.string);
	else
		strcpy (path, "listip.cfg");
...
void SV_InitOperatorCommands (void)
{
	...
	//eqds
	Cmd_AddCommand ("listip", SV_BanFilterList_f);
	Cmd_AddCommand ("loadip", SV_LoadIPList_f);
	...
	Cmd_AddCommand ("skinlist", SV_SkinList_f);
	Cmd_AddCommand ("uptime", SV_Uptime_f);

	// FTE
	Cvar_Register (&pr_tempstringcount);
	Cvar_Register (&pr_tempstringsize);
